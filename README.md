# Crawler — Free SEO Crawler & Website Audit Tool (Self-Hosted)
...
| | Crawler | Screaming Frog (free) | Screaming Frog (paid) | Sitebulb | Ahrefs Site Audit |
|---|---|---|---|---|---|
| URL limit | **Unlimited** | 500 | Unlimited | Unlimited | Per-credit |
| Price | **Free** | Free | £199 / yr | $13.50+ / mo | $129+ / mo |
| Self-hosted | ✅ | ✅ | ✅ | ✅ | ❌ cloud |
| Phones home | ❌ | ❌ | ❌ | ❌ | ✅ |
...
Each installer registers the crawler as a background service that starts on boot/login, plus a daily auto-updater that pulls the latest from this repo (with rollback on failure). Installs to `~/Crawler` (or `%USERPROFILE%\Crawler` on Windows). Browser auto-opens to `http://localhost:5002/` when done.
...
- Registers `crawler.service` so the crawler starts on every boot
- Registers `crawler-update.timer` to `git pull` + restart 2 min after every boot and once daily (auto-rolls-back on any failure)
- Prints the access URLs (`http://localhost:5002/` plus your LAN IP) and auto-opens the browser if you're on a desktop
- Saves the URLs to `~/Crawler/ACCESS_URLS.txt` for later

Useful commands after install:

```bash
systemctl status crawler                  # is it running?
sudo systemctl restart crawler            # restart
sudo systemctl disable crawler            # stop autostarting on boot
./install.sh --update-now                          # force a git-pull + restart now
systemctl list-timers | grep crawler      # when's the next auto-update?
journalctl -u crawler-update.service -n 50  # update history
sudo systemctl disable --now crawler-update.timer  # turn auto-update off
tail -f /var/log/crawler.log              # live app logs
```
...
- Clones the repo, creates a virtualenv, installs Python deps
- Registers `io.crawler.app` LaunchAgent (starts on login, restarts on crash)
- Registers `io.crawler.update` LaunchAgent (runs 2 min after login + daily at 03:30 with auto-rollback)
- Opens `http://localhost:5002/` in your default browser

Useful commands after install:

```bash
launchctl list | grep crawler                                                # is it running?
launchctl kickstart -k gui/$(id -u)/io.crawler.app                           # restart
launchctl bootout gui/$(id -u)/io.crawler.app                                # stop
rm ~/Library/LaunchAgents/io.crawler.app.plist                               # disable autostart
./install-macos.sh --update-now                                                     # force update now
tail -f ~/Library/Logs/Crawler/app.log                                       # live app logs
tail -f ~/Library/Logs/Crawler/update.log                                    # update history
```
...
- Clones the repo to `%USERPROFILE%\Crawler`, creates a virtualenv, installs Python deps
...
```powershell
& "$env:USERPROFILE\Crawler\Run.ps1"        # run in this window
& "$env:USERPROFILE\Crawler\Stop.ps1"       # stop whatever is on port 5002
& "$env:USERPROFILE\Crawler\Update.ps1"     # pull latest + reinstall deps if changed
Get-Content "$env:USERPROFILE\Crawler\autostart.log" -Tail 50 -Wait   # tail autostart/update log
```

To disable auto-start on logon, delete the `Crawler (Autostart)` shortcut from
...
The author (Nouman Sajid) accepts no liability for misuse.
