# Crawler - Windows installer (PowerShell)
#   - No admin required
#   - No Scheduled Tasks
...
$INSTALL_DIR = Join-Path $env:USERPROFILE 'Crawler'
$APP_NAME    = 'Crawler'
$PORT        = 5002
$MIN_PY_MAJ  = 3
$MIN_PY_MIN  = 10
...
# -------- Preflight --------
Write-Host '=============================================='
Write-Host ' Crawler - Windows preflight checks'
...
$runPs1 = @'
# Run the Crawler in this window.
$ErrorActionPreference = 'Stop'
...
Write-Host 'Crawler starting on http://localhost:5002/ ...'
& $py (Join-Path $PSScriptRoot 'app.py')
'@
Set-Content -Path $runPs1Path -Value $runPs1 -Encoding UTF8

$stopPs1 = @'
# Stop any Crawler python process bound to port 5002.
...
# Save URLs to a file
$urls = "Crawler - access URLs`r`nInstalled: " + (Get-Date).ToString() + "`r`n`r`nOn this computer:`r`n  http://localhost:" + [string]$PORT + "/`r`n"
if ($lanIps) {
