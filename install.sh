#!/usr/bin/env bash
# Crawler — Linux Mint installer
#   - preflight checks
#   - systemd autostart on boot
#   - daily auto-update via systemd timer (git pull + restart if changed)
#
# Usage:
#   ./install-crawler.sh              # install + enable autostart + enable daily auto-update
#   ./install-crawler.sh --check      # preflight only, no changes
#   ./install-crawler.sh --update-now # run the updater immediately (after install)
set -euo pipefail

REPO_URL="https://github.com/alfa546/Crawler.git"
INSTALL_DIR="$HOME/Crawler"
SERVICE_NAME="crawler"
UPDATE_NAME="crawler-update"
PORT=5002
MIN_PY_MAJOR=3
MIN_PY_MINOR=10
MIN_DISK_MB=500
...
[Unit]
Description=Crawler (self-hosted SEO site crawler)
...
[Unit]
Description=Crawler auto-updater (git pull + restart if changed)
After=network-online.target
Wants=network-online.target
...
[Unit]
Description=Daily check for Crawler updates
...
green " Update now:  ./install-crawler.sh --update-now"
